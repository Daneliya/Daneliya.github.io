---
url: /Java/微服务专栏/05.流控组件Sentinel/1_Sentinel微服务保护.md
---
...
sentinel官方提供了UI控制台，方便对系统做限流设置。可以在[GitHub](https://github.com/alibaba/Sentinel/releases)下载。
...
Sentinel提供了 BlockExceptionHandler 接口，可通过实现该接口来自定义异常处理逻辑。
...
public class MyBlockExceptionHandler implements BlockExceptionHandler {
   ...
   public void handle(HttpServletRequest request, HttpServletResponse response, BlockException e) throws Exception {
       log.info("Blocked by Sentinel: {}", e.getRule());
       Result result;
       if (e instanceof FlowException) {
           ...
           result = Result.error(103, "触发系统保护规则");
       } else if (e instanceof AuthorityException) {
           result = Result.error(104, "授权规则不通过");
       } else {
           result = Result.error(500, "未知错误");
       }
...
也可以使用 @SentinelResource 注解，在需要限流或降级的方法上添加@SentinelResource，并指定blockHandler 或 fallback 方法：
...
如果需要将限流或降级逻辑放到独立类中，需确保方法为静态

```java
public class CustomHandler {
   public static String handleBlock(String param, BlockException ex) {
       return "静态限流处理";
   }
}
```
...
[Sentinel 教程 | 代码酷](https://www.echo.cool/docs/category/sentinel-教程)
